...
1. **Landing → Onboarding (chat) → Dashboard (sign-in) → Control Centre (iframe)**  
   Demo users (table `ai-sahayak-users`): raju/Indore/MP, ramesh/Bhopal/MP, suresh/Jaipur/RJ, kanta/Surat/GJ, lakshmi/Hyderabad/TS. Credentials in UI; iframe gets `?retailer=raju` etc. so data is scoped.
